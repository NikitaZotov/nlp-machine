...
def get_elements_from_oriented_set(set_node: ScAddr, relation: ScAddr) -> List[ScAddr]:
    def get_next_element(access_edge: ScAddr = None):
        if access_edge:
            elem_search_result = search_next_element_template(set_node, access_edge, relation)
        else:
            keynodes = ScKeynodes()
...
def search_next_element_template(
        set_node: ScAddr, prev_element_edge: ScAddr, relation: ScAddr
) -> Optional[ScTemplateResult]:
    templ = template_next_element_of_oriented_set(set_node, prev_element_edge, relation)
    return get_first_search_template_result(templ)
