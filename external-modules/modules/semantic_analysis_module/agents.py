...
from modules.semantic_analysis_module.analyser import SemanticAnalyser

logger = get_default_logger(__name__)


class SemanticAnalysisAgent(ScAgent):
    action = ActionIdentifiers.ACTION_SEMANTIC_ANALYSIS_AGENT.value

    def register(self) -> ScEvent:
        return generate_event(
            SemanticAnalysisAgent.keynodes[QuestionStatus.QUESTION_INITIATED.value],
            common.ScEventType.ADD_OUTGOING_EDGE,
            SemanticAnalysisAgent.run_impl,
        )

    @staticmethod
    def run_impl(action_class: ScAddr, edge: ScAddr, action_node: ScAddr) -> None:  # pylint: disable=W0613
        if validate_action(SemanticAnalysisAgent.action, action_node):
            logger.info(generate_start_message(SemanticAnalysisAgent))
            keynodes = SemanticAnalysisAgent.keynodes
            try:
                ...
                logger.error(str(ex))
            logger.info(generate_finish_message(SemanticAnalysisAgent))
