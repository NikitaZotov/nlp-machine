...
class SentencesAnalysisAgent(ScAgent):
    action = ActionIdentifiers.ACTION_LEXICAL_ANALYSIS_AGENT.value

    def register(self) -> ScEvent:
        return generate_event(
            SentencesAnalysisAgent.keynodes[QuestionStatus.QUESTION_INITIATED.value],
            common.ScEventType.ADD_OUTGOING_EDGE,
            SentencesAnalysisAgent.run_impl,
        )

    @staticmethod
    def run_impl(action_class: ScAddr, edge: ScAddr, action_node: ScAddr) -> None:  # pylint: disable=W0613
        if validate_action(SentencesAnalysisAgent.action, action_node):
            logger.info(generate_start_message(SentencesAnalysisAgent))
            keynodes = SentencesAnalysisAgent.keynodes
            try:
                ...
                logger.error(str(ex))
            logger.info(generate_finish_message(SentencesAnalysisAgent))
